...
function redirectToLoginWithError(nextPath: string, intent: string, message: string) {
...
export default function AuthCallbackPage() {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            if (providerError) {
                if (!active) return
                redirectToLoginWithError(nextPath, intent, providerError)
                return
            }
            ...
            if (code) {
                ...
                if (!active) return
                if (error) {
                    redirectToLoginWithError(nextPath, intent, error.message)
                    return
                }
            }
            // Handle Implicit Flow (tokens in URL hash)
                ...
                if (hashError) {
                    if (!active) return
                    redirectToLoginWithError(nextPath, intent, hashError)
                    return
                }
                ...
                if (error || !data.session) {
                    ...
                    if (retry.error || !retry.data.session) {
                        redirectToLoginWithError(nextPath, intent, "Failed to establish session from implicit flow")
                        return
                    }
                }
            } else {
                if (!active) return
                redirectToLoginWithError(nextPath, intent, "Missing OAuth authorization code or token")
                return
            }
        ...
        const timeout = setTimeout(() => {
            if (active) {
                active = false
                window.location.href = callbackState.timeoutRedirectPath
            }
        }, 8000)
