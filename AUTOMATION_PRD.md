...
## 1. 개요 (Overview)
기존 SnapQuote가 **"견적서 작성의 고통"**을 해결했다면, 이번 확장은 **"견적서 발송 이후의 모든 귀찮은 행정 업무"**를 자동화하는 것을 목표로 합니다.
별도의 비서나 사무 직원을 고용할 여력이 없는 1인~소규모 기술자(Contractor)를 위해, **"잠자는 동안에도 일하는 보이지 않는 매니저"**를 제공합니다.
...
*   **Higher Conversion**: 까먹고 놓치는 고객에게 자동으로 연락해 수주율 상승 (팔로업)
*   **Zero Admin**: 수주 후 자재 준비, 일정 잡기, 리뷰 요청 등 반복 업무 제로화
*   **Professionalism**: 1인 기업이지만 대기업처럼 체계적인 커뮤니케이션 제공
...
### 4.1 핵심 기술 스택 (Required Skills)
이 자동화 기능을 구현하기 위해 다음 5가지 기술 요소를 활용합니다.

1.  **Serverless Backend (Supabase Edge Functions)**
    *   **역할**: 자동화의 '두뇌'. 복잡한 비즈니스 로직(언제, 누구에게, 무엇을 보낼지) 판단.
    *   **구현**: TypeScript/Deno 기반 함수 작성.

2.  **Database Scheduling (pg_cron)**
    *   **역할**: 자동화의 '심장'. 사용자가 앱을 켜지 않아도 서버에서 주기적으로 작업을 실행.
    *   **구현**: PostgreSQL 확장을 이용해 매 정각마다 `Quote Chaser` 로직 트리거.

3.  **API Integrations (Resend & Twilio)**
    *   **역할**: 자동화의 '손과 발'. 실제 메시지 전송 담당.
    *   **구현**: Resend (이메일, 무료 티어 우수), Twilio (SMS, 유료).

4.  **Queue Management (비동기 처리)**
    *   **역할**: '교통정리'. 대량 발송 시 서버 부하 분산 및 실패 시 재시도 처리.
    *   **구현**: Supabase Database Table을 Job Queue로 활용.

5.  **Frontend UI (Next.js)**
    *   **역할**: '조종석'. 사용자가 자동화 규칙을 ON/OFF 하고 설정을 변경.
    *   **구현**: React Query로 설정값 동기화, Shadcn UI로 토글 스위치 구현.
...
create table automations (
  ...
  settings jsonb -- { "delay_days": 3, "template_id": "default" }
);

-- 작업 대기열 테이블 (Job Queue)
...
1.  **Phase 2.1 (Quote Chaser)**: 가장 효과가 좋은 팔로업 기능부터 구현 (이메일만).
2.  **Phase 2.2 (Review Bot)**: 리뷰 요청 기능 추가.
3.  **Phase 2.3 (SMS 연동)**: Twilio 연동하여 문자 메시지 지원 (비용 발생으로 인한 별도 크레딧 정책 고려).
